...
function spawnBurst(state, x, y, count = 12) {
    for (
        let i = 0;
        i < count && state.agents.length < state.maxAgents;
        i += 1
    ) {
        const angle = Math.random() * Math.PI * 2;
...
function resizeCanvas(state) {
    ...
    while (
        state.agents.length <
        Math.min(state.maxAgents, Math.floor(state.maxAgents * 0.62))
        ) {
...
function updateAgents(state, dt, now) {
    ...
    for (let i = 0; i < agents.length; i += 1) {
        ...
        if (pointer.active) {
            ...
            if (distance < reach) {
                ...
                agent.vx +=
                    (dx / distance) * pull * direction +
                    (-dy / distance) * swirl;
                agent.vy +=
                    (dy / distance) * pull * direction +
                    (dx / distance) * swirl;
            }
        }
    }
    ...
    for (let i = 0; i < agents.length; i += 1) {
        ...
        for (let j = i + 1; j < agents.length; j += 1) {
            ...
            if (distanceSquared > reach * reach || distanceSquared < 0.01)
                continue;
            ...
            const forceA =
                (INTERACTION[a.type][b.type] * falloff * 0.052 - separation) *
                dt;
            const forceB =
                (INTERACTION[b.type][a.type] * falloff * 0.052 - separation) *
                dt;
    ...
    for (const agent of agents) {
        ...
        if (agent.x > state.width - margin)
            agent.vx -= (agent.x - (state.width - margin)) * 0.0035 * dt;
        if (agent.y < margin) agent.vy += (margin - agent.y) * 0.0035 * dt;
        if (agent.y > state.height - margin)
            agent.vy -= (agent.y - (state.height - margin)) * 0.0035 * dt;
        ...
        const limit =
            SPEED_LIMITS[agent.type] * (state.pointer.downRight ? 1.18 : 1);
        if (speed > limit) {
    ...
    for (let i = 0; i < agents.length; i += 1) {
        ...
        for (let j = i + 1; j < agents.length; j += 1) {
            ...
            const alpha =
                ...
                (a.type === b.type ? 0.55 : 0.38);
            ctx.globalAlpha = alpha;
            ctx.strokeStyle =
                a.type === 2 || b.type === 2 ? palette.linkHot : palette.link;
            ctx.beginPath();
            ctx.moveTo(a.x, a.y);
            ctx.lineTo(b.x, b.y);
            ctx.stroke();
...
function drawPointer(state, palette, time) {
    ...
    const modeColor = pointer.downRight
        ? palette.erase
        : pointer.downLeft
            ? palette.spawn
            : palette.cursor;
...
function initAgents() {
    ...
    const animate = (time) => {
        const dt = Math.min(
            1.8,
            Math.max(0.35, (time - state.lastTime) / 16.67),
        );
        state.lastTime = time;
